...
export const fullCvInformationResolversResolvers = {
  FullCvInformationRoot: {
    getFullCvInformation: async (
      ...
      const fullCvInformationResponse = await loaders.fullCvInformation.byUserIdInClaims.load("UserIdInClaims");

      if (!fullCvInformationResponse) return null;

      const { educationDtos, fullUserInformationDto, languageDtos,
              statusCode, otherInformationDto, userPresentationDto, 
              workExperienceDtos } = fullCvInformationResponse;
